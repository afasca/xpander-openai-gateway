...
## 顶替 Omni 内置提示词（纯净模型通道，全自动）

Omni 智能体自带约 **5.8 万 token** 的服务端系统提示词，会锁定"我是 Omni"的人设，system 指令无法覆盖。

**网关已全自动解决**：导入 Cookie 后，网关会自动在该账号下创建一个名为 `GW-Raw` 的无内置提示词裸 agent（复用网页端 `create-agent` 接口），并默认使用——多账号各自独立创建，无需任何手动选择。效果：

- 内置提示词从 ~5.8 万降到约 **2.7 万 token**（残余为 xpander agent 框架自身的执行提示词，无法去除）
- `system` 角色指令完全生效（可自定义人设）
- 账号 → agent 的映射持久化在 `.env` 的 `XP_AGENT_MAP`（重启不丢失，重复导入按名字复用不重复创建）
- WebUI「账号池管理」可查看每个账号的专属 agent 状态（已创建/创建中/失败可重试）
- 兜底：专属 agent 创建失败时，临时回落到 `XP_AGENT_ID` 指定的智能体或账号下第一个（一般是 Omni）；请求到达时若裸 agent 尚未就绪会同步等待其创建完成，保证首个请求也走纯净通道
- 代价：裸 agent 默认**没有工具**（联网搜索等）

## 酒馆（SillyTavern）预设导入

WebUI「概览 → 酒馆预设 / 人设注入」粘贴**预设 JSON**（或上传文件），网关会把其中的 system / assistant / user 条目按原顺序注入到每次请求最前面：

- 自动跳过 `enabled: false` 的条目和 `@深度` 注入条目（`injection_position≠0`）
- assistant / user 条目成为示例对话（few-shot），实测能锁人设语气
- 与请求里已有的相同内容自动去重；也支持纯文本（当作单条 system）
- 持久化在 `.env` 的 `XP_PRESET`（网关自动维护）
- **楼层（聊天记录）不用导入**：酒馆每次请求自带全部历史，网关按 `User:/Assistant:` 标签完整保留，已实测 14 层楼埋点细节 + 10 万字长文均可准确回忆

注：上游 agent 的服务端 instructions 写入接口（官方 API PATCH）实测不落盘，故预设采用网关层注入，效果等价。
